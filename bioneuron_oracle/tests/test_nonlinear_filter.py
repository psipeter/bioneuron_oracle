...
def test_nonlinear_1d(Simulator, plt):
	...
	def simulate_legendre(order):
		...
		filter_filename = 'order_%s.npz' % order
		...
		return xhat_bio, xhat_lif, x_target, times
	...
	df_list = []
	# times=np.arange(dt+t_transient, t_transient+t_test, dt)
	for order in orders:
		xhat_bio, xhat_lif, x_target, times = simulate_legendre(order)
