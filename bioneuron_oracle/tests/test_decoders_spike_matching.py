...
def sim_feedforward(Simulator):
    ...
    with nengo.Network() as network:
        ...
        if signal == 'prime_sinusoids':
            stim = nengo.Node(lambda t: prime_sinusoids(
                t, dim, t_final))
        elif signal == 'step_input':
            stim = nengo.Node(lambda t: step_input(
                t, dim, t_final, dt_nengo))
        ...
        network.probe_direct = probe_direct
    ...
    return network, decoders_bio_old, xhat_bio_old, rmse_bio_old
...
def test_new_LIF_old_decoders(Simulator, plt):
    ...
    network, decoders_bio_old, xhat_bio_old, rmse_bio_old = \
    ...
    rmse_bio_mixed = rmse(sim.data[network.probe_direct], xhat_bio_mixed)
    ...
    rmse_bio_new = rmse(sim.data[network.probe_direct], xhat_bio_new)
    ...
    plt.plot(sim.trange(), sim.data[network.probe_direct], label='oracle')
    plt.xlabel('time (s)')
    plt.ylabel('$\hat{x}(t)$')
    plt.title('decode')
...
def test_new_signal_old_decoders(Simulator, plt):
    ...
    signal_seed = 123
    with network:
        network.stim.output = lambda t: equalpower(
            t, dt_nengo, t_final, max_freq, dim,
            mean=0.0, std=1.0, seed=signal_seed)
