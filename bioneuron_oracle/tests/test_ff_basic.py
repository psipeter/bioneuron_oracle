...
def test_ff_basic_voltage():
    ...
    for bioneuron in bio.neuron_type.neurons:
        ...
        t_equilibrium = len(np.where((-67.0 < V) & (V < -63.0))[0])
        t_total = len(time)
        assert 1. * t_saturated / t_total < cutoff_sat
        assert 1. * t_equilibrium / t_total < cutoff_eq


def test_ff_basic_decode(plt):
    """
