...
def test_OracleSolver(Simulator, plt):
    """
    ...
    def sim(w_train, decoders_bio=None, plots=False):
        ...
        with nengo.Network() as model:
            ...
            probe_bio = nengo.Probe(bio, synapse=tau_neuron, solver=oracle_solver)
            probe_bio_spikes = nengo.Probe(bio.neurons, 'spikes')
            probe_direct = nengo.Probe(direct, synapse=tau_nengo)
        ...
        rmse_bio_probe = rmse(sim.data[probe_bio], sim.data[probe_direct])
