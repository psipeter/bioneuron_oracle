...
__all__ = ['train_filters_decoders_alif', 'build_filter_alif']

# def build_filter(b_s, a_s):
def build_filter_alif(zeros, poles, gain):
	"""
...
def train_filters_decoders_alif(
	network,
	Simulator,
	sim_seed,
	signals,
	...
	def get_decoders(inputs, plot=False):
		...
		if plot:
			...
			ax1.legend()
			figure.savefig('plots/deriv_filters/decodes_%s.png' %ID)
			...
			ax1.legend()
			figure.savefig('plots/deriv_filters/activities_%s.png' %ID)
		return d_bio, d_alif
	...
	for p in range(evo_popsize):
		...
		for p in poles_init:
			my_poles_bio.append(rng.uniform(0, p))  # poles must be negative
			my_poles_alif.append(rng.uniform(0, p))
	...
	for g in range(evo_gen):
		inputs = [[network, Simulator, [signals, derivatives], bio_filter_pop[p], alif_filter_pop[p]]
			for p in range(evo_popsize)]
	...
	get_decoders([network, Simulator, [signals, derivatives],
	...
	ax1.legend()
	figure.savefig('plots/deriv_filters/fitness_vs_generation.png')
